...
<body>
	...
	<header>
	<h1 style="font-family:Bell MT;color:white;">haak</h1>
...
<script>
 ...
 cartApp.controller("addToCartCtrl", function ($scope, $http){
     ...
     $scope.removeItemFromCart = function (id) {
         $http.put('http://localhost:9080/haak/categoryDropdown/productDisplay/usercart/cart/removeItem/'+id).success(function (data) {
     ...
     $scope.addItemToCart = function (id) {
         $http['delete']('http://localhost:9080/haak/usercart/cart/addItem/'+id).success(function (data) {
     ...
     $scope.refreshCartItems = function () {
         $http.get('http://localhost:9080/haak/categoryDropdown/productDisplay/usercart/cart/refreshCart/'+$scope.cartId).success(function (data) {
            $scope.cart=data;
         });
     };
     ...
     $scope.clearCartItems = function () {
     	$http['delete']('http://localhost:9080/haak/categoryDropdown/productDisplay/usercart/cart/clearCartItems/'+$scope.cartId).success(function (data)
     			{$scope.refreshCartItems()
     		});
     };
 });
